...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "energi (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork, stake)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N NRG anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/energicryptocurrency/energi";
    const std::string URL_WEBSITE = "<https://www.energi.world>";
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    //lite mode disables all Energi-specific functionality
    fLiteMode = false; // GetBoolArg("-litemode", false);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all Energi-specific functionality is disabled."));
    }
    ...
    if (privateSendClient.fEnablePrivateSend) {
        LogPrintf("Forcibly disabling PrivateSend in preparation for Gen 3 migration.");
