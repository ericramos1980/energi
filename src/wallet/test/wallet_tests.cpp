...
static const CWallet& get_wallet() {
...
static void add_coin(const CAmount& nValue, int nAge = 6*24, bool fIsFromMe = false, int nInput=0)
{
    ...
    std::unique_ptr<CWalletTx> wtx(new CWalletTx(&get_wallet(), MakeTransactionRef(std::move(tx))));
    if (fIsFromMe)
    {
...
BOOST_FIXTURE_TEST_CASE(rescan, TestChain100Setup)
{
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetImmatureBalance(), 456 * COIN / 100);
    }
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetImmatureBalance(), 228 * COIN / 100);
    }
...
BOOST_FIXTURE_TEST_CASE(importwallet_rescan, TestChain100Setup)
{
    ...
    coinbaseTxns.emplace_back(*CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()), BLOCK_TIME).CoinBase());
    coinbaseTxns.emplace_back(*CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()), BLOCK_TIME).CoinBase());
    ...
    coinbaseTxns.emplace_back(*CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()), KEY_TIME).CoinBase());
    ...
    // Import key into wallet and call dumpwallet to create backup file.
    {
        ...
        request.params.push_back(RPCGenAuthCode().c_str());
        ::pwalletMain = &wallet;
        ::dumpwallet(request);
    }
