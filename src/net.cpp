...
void CConnman::ThreadOpenConnections()
{
    ...
    if (mapMultiArgs.count("-connect") && mapMultiArgs.at("-connect").size() > 0)
    {
        for (int64_t nLoop = 0;; nLoop++)
        {
            ...
            BOOST_FOREACH(const std::string& strAddr, mapMultiArgs.at("-connect"))
            {
                ...
                OpenNetworkConnectionAsync(addr, false, NULL, strAddr.c_str());
    ...
    while (!interruptNet)
    {
        ...
        while (!interruptNet)
        {
            ...
            if (addr.GetPort() != Params().GetDefaultPort() && addr.GetPort() != GetListenPort() && nTries < ALLOW_NONDEFAULT_ATTEMPT)
                continue;
        ...
        if (addrConnect.IsValid()) {
            ...
            OpenNetworkConnectionAsync(addrConnect, (int)setConnected.size() >= std::min(nMaxConnections - 1, 2), &grant, NULL, false, fFeeler);
        } else {
            fast_mode = false;
        }
    ...
    while (true)
    {
        ...
        for (const AddedNodeInfo& info : vInfo) {
            if (!info.fConnected) {
                ...
                OpenNetworkConnectionAsync(CAddress(service, NODE_NONE), false, &grant, info.strAddedNode.c_str(), false, false, true);
                if (!interruptNet.sleep_for(DEFAULT_OUTBOUND_INTERVAL))
                    return;
            }
        }
